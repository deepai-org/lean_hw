...
//
// tb_lnp64mini_epoch.v -- the Layer-2 cross-core demo: core 0 bumps an
// epoch cell through the GP MMIO aperture and core 1 -- which holds a
// reference to the same cell and cannot write any freshness state --
// observes -STALE on its next check.
//
// Same fabric as tb_lnp64mini_dual.v (one behavioral AXI3 slave behind the
// one HP master, two cmd drivers, CORE1_HOLD), with `lnp64mini_epoch` as
// the DUT: the dual plus `epochengine` and the two `epochmmio` adapters.
//
// Expected (0x100 = "result valid", low 3 bits = Protocol.Outcome):
//   c1.r9  = 0x100 (256)  ok        -- the live handle before the bump
//   c1.r10 = 0x103 (259)  -STALE    -- the SAME handle after the bump returned
//   c0.r12 = 0x103 (259)  -STALE    -- and at volume 0 too (T-E1: "anywhere")
//   c0.r13 = 0x100 (256)  ok        -- the new epoch validates
//   c0.r15 = 0x102 (258)  -POISONED -- after a poison bump, even the CURRENT epoch
//   c0.r11 / c0.r14       the engine's measured bump-return latency, in cycles
`timescale 1ns/1ps
`ifndef TEXT1
...
`endif
    ...
    $display("EPOCH c1.r9(live check)   =%0d (want 256 = ok)", dut.c1_rf[9]);
    $display("EPOCH c1.r10(after bump)  =%0d (want 259 = -STALE)", dut.c1_rf[10]);
