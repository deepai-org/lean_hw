// lnp64mini_epoch_top.v -- ZC702 top for the dual core + the §3 EPOCH ENGINE.
// Identical to lnp64mini_dual_top except for the core instance, ID 0x53301019,
// and two extra BSCAN read registers (60/61) exposing the engine's bump
// latency counter and sequencer state for the demo's measurement.
...
module lnp64mini_epoch_top (
    input  wire       sys_clk_p,
    input  wire       sys_clk_n,
    output wire [3:0] leds
);
    localparam [31:0] ID_MAGIC = 32'h5330_1019;
    ...
    // ---- §3 epoch engine observability (demo readout) ----
    wire [15:0] o_ep_bump_cycles;
    wire [2:0]  o_ep_b_st;
    wire        o_ep_bump_busy, o_ep_bump_done0, o_ep_bump_done1;
    reg [15:0] ec0=0, ec1=0;  always @(posedge drck) begin ec0<=o_ep_bump_cycles; ec1<=ec0; end
    reg [5:0]  es0=0, es1=0;  always @(posedge drck) begin
        es0<={o_ep_bump_done1,o_ep_bump_done0,o_ep_bump_busy,o_ep_b_st}; es1<=es0; end

    lnp64mini_epoch u_dual (
        .o_ep_bump_cycles(o_ep_bump_cycles), .o_ep_b_st(o_ep_b_st),
        .o_ep_bump_busy(o_ep_bump_busy), .o_ep_bump_done0(o_ep_bump_done0),
        .o_ep_bump_done1(o_ep_bump_done1),
        .clk(sysclk), .rst(rst),
    ...
    always @(posedge update) begin
        if (sel) begin
            case (w_idx)
                ...
                7'd60: rd_reg <= {16'd0, ec1};   // engine: last bump latency (cycles)
                7'd61: rd_reg <= {26'd0, es1};   // engine: {done1,done0,busy,b_st}
