; pingpong0.s -- DUAL_SPEC ladder step 2, shared test #2 (core 0 half):
; futex ping-pong across the two cores. `turn` (first .data word, byte
; address 0x10000 = shared DDR) says whose turn it is. Core 0 runs when it
; reads 0, hands over by storing 1, and issues FUTEX_WAKE -- whose
; wake_out pulse is the OTHER core's doorbell. When it is not core 0's turn
...
.text
  ...
  LI   r2, 0             ; my turn value
  LI   r3, 1             ; hand-over value
