# conprobe.tcl -- run the guest's OWN console-write idiom on silicon.
#
# lnp64_con_put does: i = *wp (32-bit load); buf[i] = c (byte store);
# *wp = i + 1 (32-bit store). This is that loop, 16 times, in 12 instructions,
# with no NetBSD around it. Byte stores in isolation are already proven packed
# (bsprobe), so if THIS comes out strided the fault is in the idiom -- the
# load, the increment or the address arithmetic -- and it is 12 instructions
# wide instead of a 5 MB kernel.
#
...
set triples {}
set fh [open $LOOM_STATE_DIR/conprobe_words.txt r]
set i 0
...
set res [bulk_gread 0x3200000 5 64]
for {set j 0} {$j < 5} {incr j} {
  puts [format "MINI-VIEW  0x%08X = 0x%016X" [expr {0x13200000 + $j*8}] [lindex $res $j]]
}
targets -set -filter {name =~ {*Cortex-A9*#0*}}
puts "DAP-VIEW   [mrd -force 0x13200000 10]"
puts "VERDICT: packed if 0x13200008 = 0x4847464544434241 ('A'..'H')"
puts "PROBE_DONE"
