...
/-- Specialization of the shared transfer predicate to a packed capability
reference. -/
theorem transferKilled_ref_eval (σ : Loom.Hw.St) (d : DomainId)
    (slotE : Expr 4) (S : Slot)
...
/-- A packed reference outside the transferred source slot is silent in a
global kill tree already specialized to the shared transfer predicate. -/
theorem killedByCoreE_transfer_ref_zero (σ : Loom.Hw.St) (d : DomainId)
  ...
  apply bv1_ne_one.mp
  intro h
  exact hout ((transferKilled_ref_eval σ d slotE S hslot refE).mp h)

/-- The Mover abort expression after specializing the global kill tree to a
single transferred source slot. -/
...
/-- The Mover abort guard fires exactly when an active decoded job has an
endpoint in the transferred source slot. -/
theorem movKilledE_transfer_iff (σ : Loom.Hw.St) (d : DomainId)
  ...
  · rw [absMover_some σ hv, bv1_and_eq_one, bv1_or_eq_one]
    ...
    rw [transferKilled_ref_eval σ d slotE S hslot (.reg 14 "mov_src"),
      transferKilled_ref_eval σ d slotE S hslot (.reg 14 "mov_dst")]
    simp [hv]
    rfl
