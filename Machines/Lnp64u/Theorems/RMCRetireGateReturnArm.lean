...
/-- Complete fifth ladder arm: a live, class-correct reply handle that cannot
be placed in the caller returns `slotOccupied`. -/
theorem square_retire_gateReturn_blocked (m : Manifest) (hwf : m.WF)
    (hfit : Fits m) (σ : Loom.Hw.St)
    (hsync : ∀ d : DomainId, (σ.regs (Hw.drctr d) 32).toNat =
      (σ.regs "cycle" 32).toNat % (m.doms d).periodP)
    (hz : R0Zero σ) (hkc : KindCanon σ)
  ...
  have hword : (τ0.doms c.d).reg c.op.rs1 = (Hw.retW E).eval σ := by
    ...
    rw [hreg, hR]
  have hlogic : (Hw.retNZ E).eval σ = 1#1 ∧
      (Hw.transferBlocked E (Hw.retCl E) (Hw.retSel E)).eval σ = 1#1 := by
    apply (by decide : ∀ a b : BitVec 1,
      a &&& b = 1#1 → a = 1#1 ∧ b = 1#1)
  ...
  have hcls1 : (Hw.retSel E).clsOk.eval σ = 1#1 := by
    exact (by decide : ∀ a b : BitVec 1,
      a = 1#1 → a &&& ~~~b ≠ 1#1 → b = 1#1)
      ((Hw.retNZ E).eval σ) ((Hw.retSel E).clsOk.eval σ) hlogic.1
      (by simpa [E] using hclassPass)
  have hnz : (Hw.retW E).eval σ ≠ 0#32 :=
    (retNZ_eval_iff σ E).mp hlogic.1
  have hbridge : ∀ (S : Slot) (G : Gen),
      (τ0.doms E).liveCap S G = ((Hw.abs σ).doms E).liveCap S G := by
  ...
  have hslotE : (Hw.capSel E (Hw.retW E)).slot.eval σ =
      BitVec.ofNat 4 S.val := by
  ...
  have hsource : (τ0.doms E).caps S = some e := by
    ...
    | some e' =>
        ...
        · rw [if_pos hg] at alive
          have heq : e' = e := Option.some.inj alive
          rw [← heq]
  ...
  have hfreeSlot : τ0.freeSlot act.caller =
    ...
    by_cases hc : act.caller = E
    · rw [hc]
      simp [τ0, base, MachineState.setDom, Loom.Fun.update]
    · simp [τ0, base, MachineState.setDom, Loom.Fun.update, hc]
  ...
  have hnoneTransfer : τ0.transferCap E S act.caller = none :=
    transferCap_none_of_blocked σ τ0 E act.caller (Hw.retCl E)
      (Hw.retW E) S e hto hslotE acap hsource hlineage hfreeSlot hfreeCell
      hwfAbs (by simpa [Hw.retSel] using hlogic.2)
  have hcapLive : Machines.Lnp64u.Isa.capLive E ((Hw.retW E).eval σ) τ0 =
    ...
    · cases hd : Handle.decode ((Hw.retW E).eval σ) with
      | mk S' G' cls =>
          ...
          rw [hcls]
    · simpa [S, G] using alive
  have htransfer : Machines.Lnp64u.Isa.transferByHandle c.d act.caller
      ((τ0.doms c.d).reg c.op.rs1) τ0 = .err .slotOccupied τ0 := by
    rw [hword]
    exact transferByHandle_slotOccupied τ0 E act.caller
      ((Hw.retW E).eval σ) S G e hnz hcapLive hnoneTransfer
  have hserv0 : (τ0.doms c.d).serving = some gid := by
    simp [τ0, c, base, MachineState.setDom, Loom.Fun.update,
      refillPhase_serving, hserv]
  have hact0 : (τ0.gates gid).act = some act := by
  ...
  have hexec : Machines.Lnp64u.Isa.Wip.gateReturnExec c τ0 =
      .err .slotOccupied τ0 :=
    gateReturnExec_transferErr c τ0 gid act .slotOccupied hserv0 hact0 htransfer
  have hspec : corePhase m (refillPhase m (Hw.abs σ)) =
      τ0.setDom E fun ds =>
  ...
  let p2 : Hw.Check :=
    (.and (Hw.retNZ E) (.not (Hw.retSel E).live), .err .staleHandle)
  let p3 : Hw.Check :=
    (.and (Hw.retNZ E) (.not (Hw.retSel E).clsOk), .err .badCap)
  have hpre : ∀ x ∈ [p0, p1, p2, p3], x.1.eval σ ≠ 1#1 := by
    intro x hx
    simp only [List.mem_cons, List.not_mem_nil, or_false] at hx
    ...
    · subst x
      simpa [p2, E] using hstalePass
    · subst x
      simpa [p3, E] using hclassPass
  apply square_retire_gateReturn_firstError m hwf hfit σ hsync hifv hcl E
    rfl hifsel hifexcl hopc [p0, p1, p2, p3] []
    (.and (Hw.retNZ E) (Hw.transferBlocked E (Hw.retCl E) (Hw.retSel E)))
    .slotOccupied
  · rfl
  · exact hpre
  · simpa [E] using hblocked
