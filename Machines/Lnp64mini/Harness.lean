...
def smpSelftest : IO Unit := do
  ...
  -- EXT-4: the doorbell is KEYED now, so it must carry the address the
  -- thread parked on (`progDoorbell` waits on 0x2000). `dbWrong` carries a
  -- different key and must NOT wake it -- that is the whole increment.
  let db : Nat → MiniIn :=
  ...
  -- EXT-4: a doorbell on the WRONG key must leave it parked too. This is
  -- the claim the unkeyed broadcast could not make -- before EXT-4 this run
  -- woke the thread and halted, exactly like the right-key run.
