...
- The separate `ChannelTiming` premises are important in practice. They let a
  ...
  promise.
- `CertifiedIslands.includeFragment` transports each child's cached compiler
  and simulator certificates into a parent. Large hierarchical systems can
  certify only genuinely new islands instead of rechecking flattened child
  Designs.
- `System.BuiltSystem` packages the existential checked `System` with its exact
  dependent application. Generated release tools can execute assembly and
  emission fail closed without reconstructing a large kernel proof term.
- Explicit typed `System` observations promote selected registered or
  combinational island outputs through the top-level boundary. Missing or
  malformed requests are rejected instead of silently leaving child outputs
  unconnected.
- `ExternalApplication` binds exact external RTL bytes, named assumptions, and
  a contract witness to one checked System island. The Typed SoC tile now uses
  this path for its target-selected memory module; no text-level shell rewrite
  remains.
