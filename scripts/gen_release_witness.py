    ...
    if design_expr is not None:
        ...
        for batch_index, start in enumerate(
            ...
            if artifact == "Lnp64u":
                ...
                # The hybrid modules supply `semanticRegisterBehavior{i}`,
                # but not `semanticOutputBehavior{i}`, which SemanticRegs
                # references for every register. That proof does not use the
                # whole-plan machinery at all -- it is structural -- so emit
                # it here rather than leaving the batch API half-implemented.
                declarations = [
                    "-- Generated by scripts/gen_release_witness.py; DO NOT EDIT.",
                    *[
                ...
                for index in range(start, start + len(block)):
                    declarations += [
                        f"theorem semanticOutputBehavior{index} :",
                        f"    Symbolic.OutputBehaviorAt ({design_expr}) program"
                        f" {index} := by",
        ...
        # The memory obligations are split for the CI-tier single-edit gate:
        # every init block and every write port is an independent kernel
        # check, so each gets its own module and the batch phases compile
        # them in parallel. The monolithic SemanticMems ran 832 s on one
        # core; the aggregation that remains here is cheap.
        header = ["-- Generated by scripts/gen_release_witness.py; DO NOT EDIT."]
        common_opts = ["", f"namespace {namespace}", "", "open Loom.Release", "",
                       "set_option maxRecDepth 1000000",
                       "set_option maxHeartbeats 0", ""]
        base_module = f"{cert_module_prefix}.SemanticMemBase"
        semantic_mem_base = (header +
            [f"import {root_module}", "import Loom.Release.SymbolicSound",
             *[f"import {module}" for module in imports]] + common_opts)
        for memory_index, memory in enumerate(data["mems"]):
            source_name = f"sourceMemory{memory_index}"
            semantic_mem_base += [f"def {source_name} : Loom.Hw.MemDecl :=",
                f"  match ({design_expr}).mems[{memory_index}]? with",
                "  | some source => source",
                "  | none => {{ name := \"\", addrWidth := 0, dataWidth := 0,".replace("{{", "{"),
                "              init := fun _ => 0 }",
        ...
        semantic_mem_base += [f"end {namespace}", ""]
        write_if_changed(output.parent / "SemanticMemBase.lean",
                         "\n".join(semantic_mem_base))
        ...
        for memory_index, memory in enumerate(data["mems"]):
            ...
            for block_index, _ in enumerate(chunks(memory["init"], block_size)):
                ...
                init_file = (header +
                    ...
                    [f"theorem semanticMemory{memory_index}Init{block_index} :",
                     ...
                     f"end {namespace}", ""])
                stem = f"SemanticMemInit{memory_index}_{block_index:04d}"
                init_modules.append(f"{cert_module_prefix}.{stem}")
                write_if_changed(output.parent / f"{stem}.lean",
                                 "\n".join(init_file))
            init_tree = f"memoryInitTree{memory_index}"
            semantic_mems_body += [f"def {init_tree} : Rope (List Nat) :=",
            ...
            for port_index, write in enumerate(memory["writes"]):
                ...
                port_file = (header +
                    [f"import {base_module}",
                     "import Loom.Release.SymbolicDecide",
                     *[f"import {module}" for module in port_cert_modules]] +
                    common_opts +
                    [f"theorem {prefix}Certificate :",
                     ...
                     f"end {namespace}", ""])
                stem = f"SemanticMemPort{memory_index}_{port_index}"
                port_modules.append(f"{cert_module_prefix}.{stem}")
                write_if_changed(output.parent / f"{stem}.lean",
                                 "\n".join(port_file))
            ...
            semantic_mems_body += [f"def {port_roots} : List Symbolic.MemoryPortRoot := [",
                "  " + ",\n  ".join(port_entries), "]", "",
        ...
        semantic_mems = (header +
            [f"import {module}" for module in
             [base_module] + init_modules + port_modules] + common_opts +
            semantic_mems_body +
            ["def memoryRoots : List Symbolic.MemoryRoot := [",
        ...
        # A port-only variant for paths whose register certificates come from
        # elsewhere (LNP64-u uses the hybrid action cuts). Running all of
        # CertGen there paid a full whole-plan synthesis to emit one imported
        # batch; this skips RulePlans.ofRules and synthPlanRegisters entirely.
        ports_gen = [
            "-- Generated by scripts/gen_release_witness.py; DO NOT EDIT.",
            "import Tools.ReleaseCertGen", "import Tools.RuntimeSsa",
            *[f"import {design_import}" for design_import in imports],
            "", "open Loom.Hw Loom.Release", "",
            "private def writeIfChanged (path : System.FilePath)",
            ...
            f"{q(str(output.parent / 'Runtime.tsv'))}",
            "  let some mems := "
            "Tools.ReleaseCertGen.synthesizeMemsCertRuntime design program",
            "    | throw (IO.userError \"port certificate synthesis failed\")",
            "  let indexedPortBatches :=",
            "    Tools.ReleaseCertGen.indexedPortDeclarationBatchesOfMems "
            "design mems 16",
            "  for (body, index) in indexedPortBatches.zipIdx do",
            f"    writeIfChanged (s!\"{output.parent}/IndexedPortCertBatch{{index}}.lean\") <|",
            f"      {q('import Loom.Release.SymbolicCertificate' + chr(10) + 'namespace ' + namespace + chr(10) + 'open Loom.Release' + chr(10) + 'set_option maxRecDepth 1000000' + chr(10))} ++",
            f"      body ++ {q('end ' + namespace + chr(10))}",
            "  IO.println s!\"port certificates: {indexedPortBatches.length} batches\"",
            "",
        ]
        write_if_changed(output.parent / "CertGenPorts.lean",
                         "\n".join(ports_gen))
