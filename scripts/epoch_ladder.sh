...
if [ -n "${EPOCH_SYNTH_JSON:-}" ]; then
  ...
  # D37 fixed that at the design (all-zero image + the cmd-13 sweep) instead
  # of acknowledging it. Every reset image in this netlist is delivered.
  python3 scripts/check_mem_init.py \
