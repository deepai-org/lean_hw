...
route_seed=${2:-1}
case "$route_seed" in
  ''|*[!0-9]*) echo "SEED must be a positive natural number" >&2; exit 2 ;;
esac
...
lake -d "$repo_root" exe surfaceRegisteredBramEvidence "$evidence_dir"
mkdir -p "$route_dir"
input_manifest="$route_dir/route-inputs.sha256"
sha256sum \
  "$evidence_dir/system.v" \
  "$evidence_dir/clock_constraints.md" \
  "$repo_root/fpga/zc702/surface_matrix/surface_registered_bram_top.v" \
...
LOOM_SURFACE_VARIANT=registered-bram \
...
sha256sum "$route_dir/routed.dcp" "$route_dir/surface_registered_bram.bit" \
  >"$route_dir/output-artifacts.sha256"
run_id=${LOOM_RUN_ID:-surface-registered-bram-seed-$route_seed}
lake -d "$repo_root" exe surfaceMatrixVivadoSignoff registered-bram \
  "$evidence_dir" "$route_dir" "$run_id" "$route_seed"
echo "SURFACE_REGISTERED_BRAM_VIVADO_PASS seed=$route_seed directory=$route_dir"
