...
def pExpr : {w : Nat} → Expr w → StateM PSt String
  ...
  | w, .udiv a b => do fresh w s!"{← pExpr a} / {← pExpr b}"
  | w, .urem a b => do fresh w s!"{← pExpr a} % {← pExpr b}"
...
private unsafe def pExprMGo : {w : Nat} → Expr w → StateM MSt String
  ...
  | w, .udiv a b => do freshM w s!"{← pExprM a} / {← pExprM b}"
  | w, .urem a b => do freshM w s!"{← pExprM a} % {← pExprM b}"
