...
provides `sysclk`, `drck`, `rst`, `w_core`, and `loom_debug_capture` (a
one-cycle `sysclk` pulse, conventionally decoded from the command channel at
`captureCmd`); the generated function is named `loom_debug_read`. -/
structure DebugMap where
  name : String
  /-- Command index whose wrapper-visible pulse latches every tap's snapshot
  register in the source domain. The reader issues it before a read group;
  see `renderTcl`. -/
  captureCmd : Nat := 76
  taps : List DebugTap
...
/-- The source-domain **snapshot register** for one tap.

Without this stage each tap crosses live: every bit settles on its own
schedule and the destination can assemble a word the source never held
(`Loom.Hw.Cdc.torn_read_exists` is the witness). With it, the crossed value
is written only on `loom_debug_capture` and is otherwise constant, which is
exactly `Loom.Hw.Cdc.holdStable`'s hypothesis; `sample_coherent_of_stable`
then gives the destination a word the source really held, for *any* per-bit
arrival order. The two-flop chain below stays — it settles metastability;
this stage supplies coherence, which is a different property and was the one
missing. -/
private def renderSnapshot (index : Nat) (tap : DebugTap) : String :=
  s!"reg {widthDecl tap.width} loom_debug_snap_{index}_c0 = {tap.width}'d0;\n\
reg {widthDecl tap.width} loom_debug_snap_{index}_c1 = {tap.width}'d0;\n\
always @(posedge sysclk) if (loom_debug_capture) begin\n\
  loom_debug_snap_{index}_c0 <= {captureSource index 0 tap};\n\
  loom_debug_snap_{index}_c1 <= {captureSource index 1 tap};\n\
end\n"
...
always @(posedge drck) begin\n\
  loom_debug_meta_{index}_c0 <= loom_debug_snap_{index}_c0;\n\
  loom_debug_sync_{index}_c0 <= loom_debug_meta_{index}_c0;\n\
  loom_debug_meta_{index}_c1 <= loom_debug_snap_{index}_c1;\n\
...
" ++
    s!"proc debug_capture \{} \{ wr {m.captureCmd} 0; after 1 }\n"
  "# Generated by Loom.Hw.DebugMap — DO NOT HAND EDIT.\n" ++
